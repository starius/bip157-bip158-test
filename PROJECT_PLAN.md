...
The closed issue/PR sweep was also performed on 2026-05-23 to find historical
fixes in one implementation that should be cross-checked against the other.
...
### Historical Cross-Project Additions

| Source History | Scenario to Add | BIP Alignment | Cross-Check Purpose |
| --- | --- | --- | --- |
| Neutrino https://github.com/lightninglabs/neutrino/issues/3, https://github.com/lightninglabs/neutrino/pull/130, https://github.com/lightninglabs/neutrino/pull/140, and https://github.com/lightninglabs/neutrino/pull/215 | `bip157.disagreement_interrogation_matrix`: run conflicting `cfheaders`, conflicting `cfcheckpt`, wrong `cfilter`, late peer joins with bad state, and first-disagreement-at-height variants. Require exact block-derived resolution where the client claims BIP157 conflict handling. | Direct BIP157 `SHOULD` conflict-resolution coverage. | Neutrino has a long history of internal fixes here; Kyoto should be checked for the same class through black-box peers, especially because its current behavior appears weaker. |
| Neutrino https://github.com/lightninglabs/neutrino/pull/55 | `bip158.full_script_not_pushdata`: create scripts where a pushed data element appears in multiple script templates. The filter and watch logic must match the whole scriptPubKey, not arbitrary pushed bytes. | Direct BIP158 element semantics. | Ensures both implementations follow the final BIP158 whole-script rule, not an older draft behavior. |
| Neutrino https://github.com/lightninglabs/neutrino/pull/84 | `bip158.op_return_conflict_resolution`: during filter-header disagreement, use a block containing OP_RETURN outputs and ensure an honest filter missing OP_RETURN scripts is not treated as invalid. | Direct BIP158 OP_RETURN exclusion. | We already test OP_RETURN exclusion at builder level; this adds peer-punishment/conflict-resolution coverage. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/56 | `bip158.empty_filter_wire_forms`: serve zero-element filters and near-empty filters at genesis-like and non-genesis heights; require the canonical one-byte zero serialization and correct filter-header commitment. | Direct BIP158 serialization coverage. | Already partially covered; keep as a required vector scenario for both adapters. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/8 and Kyoto https://github.com/2140-dev/kyoto/pull/121 | `persistence.interrupted_cfheaders_restart`: stop the adapter while a `cfheaders` batch is being processed, restart with the same data directory, and require filter headers to catch up to block headers without deletion. | Implementation recovery; supports BIP157 liveness but is not a wire-level requirement. | Neutrino had a closed bug here; Kyoto has explicit pause/filter-sync controls that should be exercised too. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/25, https://github.com/lightninglabs/neutrino/issues/211, https://github.com/lightninglabs/neutrino/issues/255, and https://github.com/lightninglabs/neutrino/pull/283; Kyoto https://github.com/2140-dev/kyoto/pull/93 and https://github.com/2140-dev/kyoto/issues/265 | `headers.pow_difficulty_edges`: build regtest/testnet-style retarget windows, invalid `bits`, and valid edge difficulty changes. The client must reject invalid header chains and continue through valid ones. | BIP157 headers-first and proof-of-work verification. | Both projects have had header-difficulty edge cases; this test prevents treating one project's fix as universal. |
| Neutrino https://github.com/lightninglabs/neutrino/pull/344; Kyoto https://github.com/2140-dev/kyoto/pull/109, https://github.com/2140-dev/kyoto/pull/273, and https://github.com/2140-dev/kyoto/pull/282 | `headers.mtp_fork_edges`: include valid equal-MTP boundaries, fork-context MTP cases, and invalid timestamp cases. Score invalid-header acceptance red only where the assertion is consensus-header validation; otherwise score as implementation robustness. | BIP157-aligned header validation; exact MTP policy should be documented per scenario. | Captures a class of bugs seen in both projects without requiring behavior that contradicts BIP157. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/68, https://github.com/lightninglabs/neutrino/pull/174, https://github.com/lightninglabs/neutrino/pull/275, and https://github.com/lightninglabs/neutrino/pull/360; Kyoto https://github.com/2140-dev/kyoto/pull/391 and https://github.com/2140-dev/kyoto/pull/506 | `bip157.large_batch_progress_timeout`: run long chains with many `getcfheaders` and `getcfilters` sub-requests, slow-but-progressing peers, late responses after timeout, and mixed fast/slow peers. Require forward progress and no stale response corruption. | BIP157 range/checkpoint behavior plus implementation liveness. | Neutrino fixed long cfheader batch timeout semantics; Kyoto has filter-message timeout work. Both need black-box stress coverage. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/100, https://github.com/lightninglabs/neutrino/issues/107, and https://github.com/lightninglabs/neutrino/pull/176 | `bip157.cfheaders_order_and_checkpoint_boundaries`: send checkpointed `cfheaders` batches around 1000-block boundaries, out-of-order but valid late responses, and duplicate responses. The client must not write filter headers out of order or request unbounded ranges. | Direct BIP157 cfheader/checkpoint range coverage. | Complements `large_batch_progress_timeout` with ordering and boundary invariants. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/138, https://github.com/lightninglabs/neutrino/pull/145, and https://github.com/lightninglabs/neutrino/pull/180; Kyoto https://github.com/2140-dev/kyoto/issues/558 and https://github.com/2140-dev/kyoto/pull/559 | `peer.bad_data_vs_race_false_ban`: mix provably invalid compact-filter data, ordinary peer disconnects, late in-flight responses, and new block announcements. Require bans only for provable bad data; ordinary races may disconnect or retry but must not poison all honest peers. | BIP157 recommends banning bad compact-filter peers; the suite must not turn timing races into proof. | Direct cross-check between Kyoto's closed false-ban bug and Neutrino's current false-ban concern. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/166, https://github.com/lightninglabs/neutrino/pull/175, and https://github.com/lightninglabs/neutrino/pull/337 | `rescan.transient_fetch_retry_order`: fail selected filter/block fetches during catch-up, then recover. Wallet matches must be ordered by chain height and must not be duplicated or missed. | Wallet correctness built on BIP158; implementation-level unless it causes missed mandatory matches. | Adds depth beyond the current simple watch/match scenarios. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/91, https://github.com/lightninglabs/neutrino/pull/92, and https://github.com/lightninglabs/neutrino/pull/340 | `rescan.concurrent_watch_updates`: add and remove watched scripts while filters are being fetched, including after catch-up and during a reorg. Require no deadlock and correct match deltas. | Implementation concurrency; compatible with BIP158 use. | Useful for adapters that expose watch-after-sync or rescan cancellation capabilities. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/193, https://github.com/lightninglabs/neutrino/issues/65, and https://github.com/lightninglabs/neutrino/pull/194; Kyoto https://github.com/2140-dev/kyoto/issues/188 and https://github.com/2140-dev/kyoto/issues/536 | `storage.filter_cache_persistence`: fetch filters without persistence, fetch again with persistence, restart, and verify cache/store state is consistent and reorganized filters are removed. | Implementation storage; not a BIP wire requirement. | Cross-checks filter-cache and filter-store behavior requested or fixed across both projects. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/70, https://github.com/lightninglabs/neutrino/pull/317, https://github.com/lightninglabs/neutrino/pull/320, https://github.com/lightninglabs/neutrino/pull/324, and https://github.com/lightninglabs/neutrino/pull/335 | `import.sideload_headers_then_p2p_divergence`: pre-load overlapping or diverging block/filter headers, then connect to honest and adversarial peers. Require correct overlap handling, divergence rejection, and state reset before P2P sync. | Optional import-path coverage; BIP157-compatible if imported data is verified or explicitly trusted. | Helps compare Neutrino's side-header-loading history with Kyoto checkpoint/startpoint behavior. |
| Neutrino https://github.com/lightninglabs/neutrino/pull/23 and Kyoto https://github.com/2140-dev/kyoto/pull/528 | `bip157.wrong_filter_type_response`: request basic filters but have a peer respond with another filter type or malformed type; require ignore, disconnect, or ban according to whether the peer sent provably invalid data. | Direct BIP157 filter-type handling. | Ensures support for future filter-type extensibility does not weaken current basic-filter behavior. |
| Neutrino https://github.com/lightninglabs/neutrino/pull/124 and https://github.com/lightninglabs/neutrino/issues/257; Kyoto https://github.com/2140-dev/kyoto/pull/548 and https://github.com/2140-dev/kyoto/pull/549 | `blocks.witness_prevout_matrix`: cover P2WPKH, P2WSH, nested segwit, and P2TR spends with and without witness block retrieval enabled. Require correct spend matches where the adapter claims witness-aware or exact prevout verification. | BIP158 prevout inclusion plus implementation capability declaration. | Connects Neutrino's spent-script derivation work and taproot limitation with Kyoto's optional witness-block request path. |
| Neutrino https://github.com/lightninglabs/neutrino/issues/212 and https://github.com/lightninglabs/neutrino/issues/223; Kyoto https://github.com/2140-dev/kyoto/pull/541 and https://github.com/2140-dev/kyoto/pull/565 | `config.restricted_peer_set_and_onion`: in deterministic mode, configure valid, invalid, DNS, and onion-style peers. The client must not use uncontrolled discovery when disabled and must continue with reachable configured peers. | Implementation/network privacy; compatible with BIP157 multi-peer guidance. | Cross-checks deterministic adapter requirements and avoids accidental external connections in the suite. |
| Kyoto https://github.com/2140-dev/kyoto/issues/544 | `services.network_limited_near_tip`: connect to a peer advertising compact filters plus `NODE_NETWORK_LIMITED` near the chain tip and verify the client can use it only for ranges the peer is allowed to serve. | Bitcoin P2P service-bit behavior; compatible with BIP157 serving. | Optional scenario for implementations that support pruned/filter-serving peers. |
| Kyoto https://github.com/2140-dev/kyoto/issues/281, https://github.com/2140-dev/kyoto/issues/537, https://github.com/2140-dev/kyoto/pull/539, and https://github.com/2140-dev/kyoto/pull/576 | `api.headers_only_and_rescan_from`: optional adapter capability for header-only sync, explicit chain-tip query, and rescan from a chosen height. It must not mark the wallet synced if filters needed for watched scripts were intentionally halted. | API/testability; not a BIP157 requirement unless advertised as wallet sync. | Improves adapter ergonomics and checks that partial sync modes do not masquerade as full filter sync. |
| Kyoto https://github.com/2140-dev/kyoto/pull/570; Neutrino https://github.com/lightninglabs/neutrino/issues/6 and https://github.com/lightninglabs/neutrino/issues/67 | `chain.sendheaders_tip_tracking`: after initial sync, announce new blocks via BIP130 `headers` rather than `inv`, including a reorg. The client should use headers-first logic and not trust advertised peer heights. | BIP130 optional transport for BIP157 headers-first operation; skip unless supported. | Kyoto has merged sendheaders support; Neutrino tracks it as open, so the suite should make this capability explicit. |
